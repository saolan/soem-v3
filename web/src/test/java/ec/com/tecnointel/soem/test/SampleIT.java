package ec.com.tecnointel.soem.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "soem-ear.ear");

        // Current directory is the root of the "soem-web" project. Go up one level
        // and enter the "ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../ejb/target/soem-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "soem-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/soem-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "soem-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("ec.com.tecnointel.soem.test");
